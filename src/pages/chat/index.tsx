...
export default function Chat() {
  ...
  const usersQuery = trpc.proxy.user.getAll.useQuery();
  const createConversationMutation =
    trpc.proxy.chat.createConversation.useMutation();
  const sendMessageMutation = trpc.useMutation(["chat.sendMessage"]);

  const users = (usersQuery.data?.users || []).filter(
    (user) => user.id !== sesh?.user?.id
  );
  ...
  trpc.useQuery(
    ["chat.getFull", { conversationId: activeConversation?.id as string }],
    {
  ...
  useSubscribeToEvent(
    `chat-message`,
    (data: { message: ChatMessage; authorId: string }) => {
