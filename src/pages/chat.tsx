...
export default function Chat() {
  ...
  const users = (usersQuery.data?.users || []).filter(
    (user) => user.email !== sesh?.user?.email
  );
  ...
  useEffect(() => {
    if (activeConversationId) {
      ...
      pusher?.unbind_all();
      pusher?.connect();
      var channel = pusher?.subscribe(activeConversationId);
      channel?.bind(
        "chat-message",
        function (data: { message: ChatMessage; authorEmail: string }) {
          console.log(data.authorEmail, sesh?.user?.email, data);
          if (data && data.authorEmail !== sesh?.user?.email) {
            console.log("receivedMessage");
            updateLastMessage(data.message);
            activeConversation?.messages.push(data.message);
          }
        }
      );
    }
